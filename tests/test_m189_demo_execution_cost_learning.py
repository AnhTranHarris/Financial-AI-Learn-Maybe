...
from dusty.broker_calibration import CalibrationStatus
...
def reconciliation(
    ...
    *,
    ...
    price = fill_price if fill_price is not None else expected + (
        0.00001 * (index % 4) if row.side is TradeSide.LONG else -0.00001 * (index % 4)
    )
...
class M189DemoExecutionCostLearningTests(unittest.TestCase):
    def test_only_partial_or_filled_m188_evidence_can_become_sample(self) -> None:
        ...
        sample = sample_from_reconciliation(
            row, filled, broker_profile_fingerprint=BROKER, point_size=0.00001
        )
        self.assertEqual(sample.reconciliation_fingerprint, filled.fingerprint)
        self.assertAlmostEqual(sample.fill_fraction, 1.0)
        pending = reconciliation(
            row, 2, status=ReconciliationStatus.PENDING, fill_fraction=0.0, fill_price=1.1002
        )
        with self.assertRaisesRegex(ValueError, "PARTIAL/FILLED"):
            sample_from_reconciliation(
                row, pending, broker_profile_fingerprint=BROKER, point_size=0.00001
            )

    def test_no_samples_is_uncalibrated_and_never_invents_metrics(self) -> None:
        learned = learn_demo_execution_costs(
            (), broker_profile_fingerprint=BROKER, symbol="EURUSD"
        )
    ...
    def test_thin_demo_sample_remains_insufficient(self) -> None:
        pairs = tuple(
            sample_pair(i, TradeSide.LONG if i % 2 == 0 else TradeSide.SHORT)
            for i in range(6)
        )
        learned = learn_demo_execution_costs(
            pairs, broker_profile_fingerprint=BROKER, symbol="EURUSD"
        )
    ...
    def test_multi_day_both_side_demo_evidence_reuses_m165_and_adds_latency_fill_metrics(self) -> None:
        ...
        self.assertGreater(
            learned.first_fill_latency_p95_ms,
            learned.first_fill_latency_p50_ms,
        )
    ...
    def test_partial_fills_reduce_empirical_fill_fraction_without_becoming_strategy_failure(self) -> None:
        ...
        for i in range(30):
            ...
            if i % 5 == 0:
                rec = ExecutionReconciliation(
                    ...
                    rec.intent_hash,
                    rec.shadow_fingerprint,
                    rec.m187_receipt_fingerprint,
                    rec.broker_evidence_fingerprint,
                    rec.observed_at,
                    rec.expected_price,
                    rec.filled_volume * 0.5,
                    0.5,
                    rec.weighted_average_fill_price,
                    rec.adverse_slippage_price,
                    rec.adverse_slippage_fraction,
                    rec.first_fill_latency_ms,
                    rec.last_fill_latency_ms,
                    rec.commission,
                    rec.swap,
                    rec.fee,
                    rec.order_tickets,
                    rec.deal_tickets,
                    rec.position_tickets,
                    rec.reasons,
                    rec.evidence_fingerprints,
                )
                sample = DemoExecutionCostSample(
                    sample.broker_profile_fingerprint,
                    rec.fingerprint,
                    sample.shadow_fingerprint,
                    sample.symbol,
                    sample.side,
                    sample.point_size,
                    sample.captured_bid,
                    ...
                    rec.filled_volume,
                    0.5,
                    sample.first_fill_latency_ms,
                    sample.last_fill_latency_ms,
                    sample.commission,
                    sample.swap,
                    sample.fee,
                )
        ...
        learned = learn_demo_execution_costs(
            tuple(pairs), broker_profile_fingerprint=BROKER, symbol="EURUSD"
        )
        self.assertEqual(learned.status, DemoCostLearningStatus.CALIBRATED)
    ...
    def test_duplicate_or_identity_drift_fails_closed(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "duplicate"):
            learn_demo_execution_costs(
                (pair, pair), broker_profile_fingerprint=BROKER, symbol="EURUSD"
            )
        source = pair[0]
        wrong = DemoExecutionCostSample(
            fp("other-broker"),
            source.reconciliation_fingerprint,
            source.shadow_fingerprint,
            source.symbol,
            source.side,
            source.point_size,
            source.captured_bid,
            ...
            source.fill_price,
            source.filled_volume,
            source.fill_fraction,
            source.first_fill_latency_ms,
            source.last_fill_latency_ms,
            source.commission,
            source.swap,
            source.fee,
        )
        with self.assertRaisesRegex(ValueError, "mix broker"):
            learn_demo_execution_costs(
                ((wrong, pair[1]),), broker_profile_fingerprint=BROKER, symbol="EURUSD"
            )
