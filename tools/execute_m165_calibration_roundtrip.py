...
def main() -> int:
    parser = argparse.ArgumentParser(description="One-shot M165 calibration round trip through the existing M187 Demo boundary")
    ...
    if assessment.status is not NativeDemoPreflightStatus.READY:
        raise PermissionError(f"native Demo preflight blocked: {assessment.blockers}")
    if snapshot.symbol_spec_fingerprint != binding.symbol_spec_fingerprint:
        raise PermissionError("native symbol specification drifted since calibration plan")
    ...
    if identity.fingerprint != binding.session_fingerprint:
        raise PermissionError("Demo session identity drifted since calibration plan")
    ...
    intent = OrderIntent(
        ...
        target_price=None,
        approved_risk_fraction=binding.actual_risk_fraction,
        allowed_loss=binding.execution_allowed_loss_cash,
        pm_approved=True,
    ...
    if intent.intent_hash != binding.intent_hash:
        raise PermissionError("reconstructed executable intent does not match calibration plan")
    ...
    if not entry_preflight.passed:
        raise PermissionError(f"fresh entry preflight failed: {entry_preflight.reasons}")
    if entry_preflight.loss_at_stop > binding.execution_allowed_loss_cash + 1e-9:
        raise PermissionError("fresh native loss exceeds measured execution binding")
    if not all(math.isfinite(value) and value > 0 for value in (entry_preflight.bid, entry_preflight.ask)):
        raise PermissionError("entry preflight did not preserve a complete native quote")
    ...
    producer = _digest(("dusty-m165-calibration-roundtrip-operator-v1", expected))
    ...
    output: dict[str, object] = {
        "protocol": "dusty-m165-calibration-roundtrip-execution-v1",
        "status": "started",
        ...
        "session_fingerprint": binding.session_fingerprint,
        "symbol_spec_fingerprint": binding.symbol_spec_fingerprint,
    ...
    try:
        valid_until = min(binding.expires_at, entry_checked_at + timedelta(minutes=5))
        if valid_until <= entry_checked_at:
            raise PermissionError("entry permit would be expired")
        entry_permit = M187CalibrationPermit(
            ...
            "open",
            binding.volume_lots,
            binding.execution_allowed_loss_cash,
            1,
            entry_checked_at,
            valid_until,
        )
        ...
        if entry_receipt.execution.state is not ExecutionState.FILLED or entry_receipt.execution.deal_ticket <= 0:
            ...
            _atomic_write_json(Path(args.output).resolve(), output)
        ...
        close_intent = PositionActionIntent(
            ...
            current_stop=float(getattr(position, "sl", 0.0) or binding.stop_price),
            new_stop=0.0,
